<script lang="ts">
  ...
  $: cleanSizes = product.isOnDemand ? ['PEDIDO'] : [...new Set(product.product_variants.filter(v => v.stock_qty > 0).map(v => v.size.split(' ')[0]))];

  // Mensaje de WhatsApp
  $: wppMessage = `https://wa.me/${PHONE}?text=${encodeURIComponent(`Hola! Consulto por ${product.name} (${product.stud_type} ${product.lace_type})`)}`;
...
<article class="bg-carbon border border-white/5 flex flex-col overflow-hidden group">
  <a href="/catalog/{product.id}" class="relative aspect-square bg-obsidian overflow-hidden block">
    ...
    <div class="absolute top-3 left-3 flex gap-1.5">
      <span class="px-2 py-1 text-[8px] font-bold bg-volt text-obsidian uppercase">{badge}</span>
  ...
  <div class="p-5 flex flex-col flex-1">
    <div class="flex justify-between items-start mb-2">
      ...
      <span class="text-white/40 font-mono text-[8px] uppercase">AR: {cleanSizes.join(', ')}</span>
    </div>
    ...
    <p class="text-volt font-heading text-2xl mb-4">${product.price.toLocaleString('es-AR')}</p>
